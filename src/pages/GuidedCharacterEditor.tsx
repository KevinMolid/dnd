...
const getTraitSummaryLines = (trait: Trait): string[] => {
  ...
  if (trait.notes?.length) lines.push(...trait.notes);
...
const GuidedDndCharacterEditor = () => {
  ...
  const activeAllTraits = useMemo(() => {
    return getAllCharacterTraits({
      ...
      backgroundId,
      originFeatId: backgroundGrantedFeatId,
      level: originalCharacter?.level ?? 1,
      choices: {
        classSkillChoices,
  ...
  useEffect(() => {
    const loadCharacter = async () => {
      ...
      try {
        ...
        const data = snap.data() as CharacterSheetData;
        setOriginalCharacter(data);

        setName(data.name ?? "");
        setImageUrl(data.imageUrl ?? "");
        setShowImageUrlInput(!!data.imageUrl?.trim());
        setClassId(data.classId ?? classes[0]?.id ?? "");
        setSpeciesId(data.speciesId ?? species[0]?.id ?? "");
        setBackgroundId(data.backgroundId ?? backgrounds[0]?.id ?? "");
        setAlignment(data.alignment ?? "");
        setNotes(data.notes ?? "");
        setAbilityScores(data.abilityScores ?? defaultAbilityScores);

        setClassSkillChoices(data.choices?.classSkillChoices ?? []);
        setSpeciesTraitChoices(data.choices?.speciesTraitChoices ?? {});

        setBackgroundBonusPlus2(
          data.choices?.backgroundAbilityBonuses?.plus2 ??
            backgrounds[0]?.abilityOptions[0] ??
            "str",
        );
        setBackgroundBonusPlus1(
          data.choices?.backgroundAbilityBonuses?.plus1 ??
            backgrounds[0]?.abilityOptions[1] ??
            backgrounds[0]?.abilityOptions[0] ??
            "dex",
        );

        setBackgroundToolChoice(data.choices?.backgroundToolChoice ?? "");
        setBackgroundFeatCantripChoices(
          data.choices?.backgroundFeatCantripChoices ?? [],
        );
        setBackgroundFeatSpellChoices(
          data.choices?.backgroundFeatSpellChoices ?? [],
        );

        setRogueExpertiseChoices(data.choices?.rogueExpertiseChoices ?? []);
        setRogueBonusLanguage(data.choices?.rogueBonusLanguage ?? "common");
  ...
  const toggleClassSkill = (skill: SkillId) => {
    setClassSkillChoices((prev) => {
      ...
      if (prev.length >= classSkillChoiceCount) return prev;
      return [...prev, skill];
    });
  };
  ...
  const toggleRogueExpertise = (choice: SkillId | "thieves-tools") => {
    setRogueExpertiseChoices((prev) => {
      if (prev.includes(choice)) return prev.filter((item) => item !== choice);
      if (prev.length >= 2) return prev;
      return [...prev, choice];
    });
  };
  ...
  const validateForm = () => {
    ...
    if (!classDef) return "Please choose a valid class.";
    if (!backgroundDef) return "Please choose a valid background.";
    ...
    for (const choice of speciesChoices) {
      ...
      if (choice.choose === 1) {
        ...
        if (!isValid) return `Please choose a valid option for ${choice.name}.`;
      } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!characterId || !originalCharacter) {
      setError("Missing character to update.");
      return;
    }
    ...
    try {
      ...
      await updateDoc(doc(db, "characters", characterId), {
        ...
        classId: updatedCharacter.classId,
        speciesId: updatedCharacter.speciesId,
        backgroundId: updatedCharacter.backgroundId,
        originFeatId: updatedCharacter.originFeatId,
        abilityScores: updatedCharacter.abilityScores,
        alignment: updatedCharacter.alignment,
        notes: updatedCharacter.notes,
        choices: updatedCharacter.choices,
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400 sm:text-base">
            Update your D&amp;D 2024 character and save changes.
          </p>
        </div>
          ...
          <aside className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-xl sm:p-6">
            ...
            <div className="mt-6 space-y-3">
              ...
              <button
                type="button"
                onClick={() =>
                  navigate(characterId ? `/characters/${characterId}` : "/")
                }
...
export default GuidedDndCharacterEditor;
