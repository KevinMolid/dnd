...
const RewardItemsModal = ({
  ...
  const handleSubmit = async () => {
    const parsedGold = Math.max(0, Math.floor(Number(gold) || 0));
    ...
    if (
      ...
      (parsedGold <= 0 && normalizedItems.length === 0)
    ) {
      return;
    }
  ...
  const canSubmit =
    selectedCharacterIds.length > 0 &&
    (Number(gold) > 0 || selectedItems.length > 0) &&
    !submitting;
        ...
        <div className="flex items-start justify-between gap-4 border-b border-white/10 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">
              Search for items, choose quantities, and reward them to players in
              this campaign.
            </p>
          </div>
          ...
          <div className="flex min-h-0 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
              ...
              <section className="mt-6">
                <label className="block text-sm font-semibold text-white">
                  Gold to add to each selected character
                </label>
                <input
                  type="number"
                  min={0}
                  ...
                  className="mt-3 w-full rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-white outline-none transition focus:border-emerald-400/40"
                />
              </section>
            ...
            <div className="border-t border-white/10 px-6 py-4">
              <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-zinc-300">
                ...
                <p className="mt-1">
                  <span className="font-semibold text-white">Gold each:</span>{" "}
