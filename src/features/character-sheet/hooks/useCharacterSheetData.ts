...
export const useCharacterSheetData = (
      ...
      async () => {
        ...
        try {
          ...
          if (
            ...
            const nextCampaignItemsById =
              Object.fromEntries(
                campaignItemsSnap.docs.map(
                  (
                    docSnap,
                  ) => {
                    const campaignItem =
  ...
  const handleSetHeroicInspiration =
    async (
      ...
      try {
        ...
        setError(
          "Failed to update Heroic Inspiration.",
        );
      }
    };

  const handleSetDeathSaves =
    async (
      ...
      const normalized: DeathSaves =
        {
          successes:
            clampDeathSave(
              nextDeathSaves.successes,
            ),

          failures:
            clampDeathSave(
              nextDeathSaves.failures,
            ),
        };
  ...
  const derived =
    useMemo<CharacterSheetDerived | null>(
      () => {
        if (
          !character
        ) {
        ...
        const strMod =
          getAbilityModifier(
            finalAbilityScores.str,
          );

        const dexMod =
          getAbilityModifier(
            finalAbilityScores.dex,
          );

        const wisMod =
          getAbilityModifier(
            finalAbilityScores.wis,
          );
        ...
        const dragonbornBreathWeaponDc =
          dragonbornAncestry
            ? 8 +
              proficiencyBonus +
              conMod
        ...
        const dragonbornBreathWeaponDamage =
          dragonbornAncestry
            ? getDragonbornBreathWeaponDice(
                character.level,
              )
        ...
        const fallbackSkillProficiencies =
          unique<SkillId>([
            ...(background
              ?.skillProficiencies ??
              []),
        ...
        const skillProficiencies =
          unique<SkillId>([
            ...
            ...((character.skillProficiencies as
              | SkillId[]
              | undefined) ??
              []),
        ...
        const savingThrowProficiencies =
          unique<AbilityKey>([
            ...
            ...((character.savingThrowProficiencies as
              | AbilityKey[]
              | undefined) ??
              []),
        ...
        const equippedArmorClass =
          getCharacterArmorClassFromEquipment(
            {
              dexterityScore:
                finalAbilityScores.dex,
                  ...
                  .filter(
                    (
                      ...
                      if (
                        ...
                        return weaponProficiencies.includes(
                          "simple-weapons",
                        );
                      }
        ...
        const speciesTraitResistances =
          getTraitResistances(
            speciesTraits,
          );

        const speciesGrantedSpells =
          getTraitGrantedSpells(
            speciesTraits,
            character.level,
          ).map(
        ...
        const selectedTieflingLegacyTraitId =
          typeof selectedTieflingLegacyId ===
          "string"
        ...
        const groupedTieflingLegacySpells =
        ...
        const hasBackgroundMagicInitiate =
          character.originFeatId ===
            "magic-initiate" &&
          !!backgroundFeatSpellListId &&
          (backgroundFeatCantripChoices.length >
            0 ||
            backgroundFeatSpellChoices.length >
              0);

        const magicInitiateSpellcasting =
          hasBackgroundMagicInitiate
            ? {
                id:
                  "background-magic-initiate",

                name:
                  `Magic Initiate (${formatLabel(
                    backgroundFeatSpellListId!,
                  )})`,
        ...
        const selectedCantripCount =
          allKnownCantripIds.length +
          backgroundFeatCantripChoices.length;

        const selectedLeveledSpellCount =
          collectedSpellChoices
            .spellSelections
            .length +
          backgroundFeatSpellChoices.length;
        ...
        const derivedKnownSpells: CharacterSpell[] =
          [
            ...
            ...collectedSpellChoices.spellSelections.map(
              (
                selection,
              ) => ({
                spellId:
            ...
            ...backgroundFeatCantripChoices.map(
              (
                spellId,
              ) => ({
                spellId,
            ...
            ...backgroundFeatSpellChoices.map(
              (
                selection,
              ) => ({
                spellId:
        ...
        const spells =
          unique(
            nonSpeciesDerivedKnownSpells.map(
              (
                spell,
              ) =>
                spell.spellId,
        ...
        const speciesGrantedFeatTraits =
          dedupeTraits(
            getSpeciesGrantedFeatIds(
              character.speciesId,
              character.choices,
            ).flatMap(
              (
        ...
        const initiativeBreakdownParts =
          [
            ...
            0
              ? [
                  `+${alertInitiativeBonus} Alert`,
                ]
              : []),
          ];

        const initiativeBreakdown =
          initiativeBreakdownParts.join(
            " ",
          );
          ...
          [
            {
              ...
              subtitle:
                "Ancestry, senses, resistances, innate gifts",
...

            {
              ...
              title:
                `${classDef?.name ?? "Class"} Features`,

              subtitle:
                "Features gained from your class levels",
...

            {
              ...
              title:
                `${subclassDef?.name ?? "Subclass"} Features`,

              subtitle:
                "Specialized features from your subclass",
...

            {
              ...
              title:
                `${background?.name ?? "Background"} Traits`,

              subtitle:
                "Background benefits and granted origin feature",
...

            {
              ...
              title:
                "Feats & Special Training",

              subtitle:
                "Origin feat, species-granted feats, and level-up feats",
        ...
        const groupedTraitIds =
          new Set(
            groupedTraits.flatMap(
              (
                group,
              ) =>
                group.traits.map(
          ...
          groupedTraits.push(
            {
              ...
              subtitle:
                "Rules content not yet assigned to a source group",
          ...
          featName:
            character.originFeatId
              ? character.originFeatId ===
                  "magic-initiate" &&
                backgroundFeatSpellListId
                ? `${featsById[character.originFeatId]?.name ?? character.originFeatId} (${formatLabel(
                    backgroundFeatSpellListId,
                  )})`
                : featsById[
                    character
