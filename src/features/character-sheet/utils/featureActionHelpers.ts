...
const dedupe = (
  entries: PlayPanelFeatureAction[],
) => {
  const seen =
    new Set<string>();
...
export const collectFeatureActions = (
  ...
  for (
     ...
     *
     * Example:
     * Nature's Sanctuary is activated with an Action, but moving the existing
     * sanctuary is a Bonus Action.
     */
    for (
      const action of
      feature.actions ??
      ...
      push(
        ...
        {
          id:
            `feature-${feature.id}-action-${action.id}`,
